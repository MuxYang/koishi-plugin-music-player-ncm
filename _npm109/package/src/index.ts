...
export function apply(ctx: Context, config: Config) {
  async function revokeMessage(session: Session, messageId: string) {
    // 优先使用 OneBot 的撤回接口（与 onebot-manager 的实现一致）
    const onebot = (session as any).onebot as { deleteMsg?: (id: string) => Promise<any> } | undefined
    if (onebot?.deleteMsg) {
      await onebot.deleteMsg(messageId).catch(() => {})
      return
    }

    // 非 OneBot 环境兜底
    if (session.channelId) {
      await session.bot.deleteMessage(session.channelId, messageId).catch(() => {})
    }
  }
  ...
  ctx.command('ncmget <keyword:text>', '获取网易云音乐')
    ...
    .action(async ({ session }, keyword) => {
      ...
      const forcedSendFormat: Config['sendFormat'] | undefined = (() => {
        const options = (session as any).argv?.options as any
        if (!options) return
      ...
      try {
        ...
        // 只有一个结果，直接获取
        if (results.length === 1) {
          return await handleSongRequest(session, results[0], forcedSendFormat)
        }
        ...
        const message = config.mergeSearchResults
          ? await session.send(session.text('commands.ncmget.messages.search-results', [resultText]))
          : await session.send(h('message', { forward: false }, [
              h('text', { content: session.text('commands.ncmget.messages.search-prompt') }),
              h('text', { content: resultText })
            ]))
        ...
        const timeout = setTimeout(() => {
          ...
          // 超时后撤回搜索列表消息
          void revokeMessage(session, message[0])
  ...
  ctx.middleware(async (session, next) => {
    ...
    if (index === -1) {
      clearSearchSession(sessionKey)
      // 撤回搜索列表消息
      await revokeMessage(session, searchSession.messageId)
      return session.text('commands.ncmget.messages.search-cancelled')
    }
    ...
    if (index >= 0 && index < searchSession.results.length) {
      ...
      if (session.messageId) {
        await revokeMessage(session, session.messageId)
      }
      
      return await handleSongRequest(session, searchSession.results[index], searchSession.sendFormat)
    }
    ...
    try {
      ...
      if (cacheEntry?.cached && cacheEntry.cachePath) {
        try {
          ...
          // 文件存在，直接发送
          return await sendCachedSong(session, cacheEntry, sendFormatOverride)
        } catch {
          // 文件不存在，更新数据库
      ...
      if (!urlInfo || !urlInfo.url) {
        return session.text('commands.ncmget.messages.song-unavailable')
      }
      ...
      // 发送歌曲
      return await sendCachedSong(session, cacheData as MusicCache, sendFormatOverride)

    } catch (error) {
      logger.error('获取歌曲失败:', error)
      return session.text('commands.ncmget.messages.download-error')
    }
  }
  ...
  async function sendCachedSong(session: Session, cache: MusicCache, sendFormatOverride?: Config['sendFormat']) {
    try {
      ...
      // 使用 h.file + file:// 协议发送本地文件
      // 注意：这里发送的是“文件”，不是“语音/音频消息”。
      ...
      if (format === 'audio') {
        return session.send(h.audio(fileUrl, { title: filename }))
      }

      return session.send(h.file(fileUrl, { title: filename }))
    } catch (error) {
      logger.error('发送歌曲失败:', error)
      return session.text('commands.ncmget.messages.send-error')
    }
  }
}
